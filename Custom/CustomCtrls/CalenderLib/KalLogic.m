...
- (id)initPopForDate:(NSDate *)date
{
    if ((self = [super init])) {
        monthAndYearFormatter = [[NSDateFormatter alloc] init];
        [monthAndYearFormatter setDateFormat:@"MMM yyyy"];
        [self moveToMonthForDate:date];
    }
    return self;
}
...
- (NSUInteger)numberOfDaysInPreviousPartialWeek
{
    ...
    if ([dataKeeper.m_mySettingInfo.startweek integerValue] == 0) {
        return ([self.baseDate cc_weekday] + 5) % 7;
    }
...
//    if([self.daysInFinalWeekOfPreviousMonth count] < 5)
        return 42 - [self.daysInFinalWeekOfPreviousMonth count] - [self.daysInSelectedMonth count];
...
- (NSArray *)calculateDaysInFinalWeekOfPreviousMonth
{
  ...
  NSDateComponents *c = [beginningOfPreviousMonth cc_componentsForMonthDayAndYear];
    ...
    NSLog(@"N = %d, partialDays = %d", n, numPartialDays);
